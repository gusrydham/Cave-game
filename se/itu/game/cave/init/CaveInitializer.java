 ...
 * We translate the DbRoom's ID to an actual Room.
  ...
  /*
   * Adding rule for bird,
   * If player has rod in the inventory,
   * it cant get picked up and RuleViolationException is thrown.
   * If player dont have cage in inventory,
   * it cant pick up bird and RuleViolationException is thrown.
   */
  ...
  private void addRules() {
    ...
    /*
     * Adding rule for Pirate Chest,
     * If player dont have all the 4 keys in inventory,
     * chest can't be picked up and a RuleViolationException is thrown
     */
    ...
    /*
     * Adding rule for snake,
     * If player have dropped down cage and bird in room 19,
     * the snake will leave
     * A new room will open up, south is set to room 29.
     */
    ...
    RuleBook.addRoomRule(getRoomById(19), new RoomRule(getRoomById(19), "There's a Snake blocking the South Exit") {
        ...
        public void apply() {
          if(room.things().contains(Things.get("Cage")) &&
            ...
            this.changeDescription("There's no Snake here");
    ...
    /*
     * Adding rule for dragon,
     * If player have dropped down Gold, Silver, Jewelry and Diamonds in room 120,
     * the dragon will leave along with the items dropped.
     * A Glass Key will appear
     */
    ...
    RuleBook.addRoomRule(getRoomById(120), new RoomRule(getRoomById(120), "There's a greedy Dragon here.") {
        ...
        public void apply() {
          if(room.things().contains(Things.get("Gold")) &&
            ...
            this.changeDescription("A Glass Key magically appears. The Dragon left with the valuables.");
            try {
      ...
      /*
      * If the player chooses to pick up the pirate chest in room 250
      * the game will be won and all the buttons are grayed out.
      */

      RuleBook.addRoomRule(getRoomById(250), new RoomRule(getRoomById(250), ""){
        @Override
        public void apply() {
          List<Thing> inventory = Player.getInstance().inventory();
          if(inventory.contains(Things.get("Pirate Chest"))){
            this.changeDescription("CONGRATULATIONS!\n You won the game! Restart to play again.");
            room.setConnectingRoom(SOUTH, null);
            room.setConnectingRoom(NORTH, null);
            room.setConnectingRoom(EAST, null);
            room.setConnectingRoom(WEST, null);
          }
        }
      });

      /*
      * Rules for rooms 63 and 38. If the player go to the rooms with ID 63 or 38
      * the game will be lost.
      * All buttons are grayed out, can not be pressed after "death".
      */

      RuleBook.addRoomRule(getRoomById(63), new RoomRule(getRoomById(63), "You slip on a rock and fall on your head and die and painful death.\n GAME OVER - Restart to try again."){
        @Override
        public void apply() {
          room.setConnectingRoom(SOUTH, null);
          room.setConnectingRoom(NORTH, null);
      ...
      RuleBook.addRoomRule(getRoomById(38), new RoomRule(getRoomById(38), "A giant boulder falls on your face and you are crushed to death.\n GAME OVER - Restart to try again."){
        @Override
        public void apply() {
          room.setConnectingRoom(SOUTH, null);
          room.setConnectingRoom(NORTH, null);
